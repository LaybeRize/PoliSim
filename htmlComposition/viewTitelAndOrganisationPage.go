...
func GetViewTitelPage() Node {
	listing := make([]Node, len(dataExtraction.TitleGroupMap))
	counter := 0
	for outer, item := range dataExtraction.TitleGroupMap {
		innerCounter := 0
		innerListing := make([]Node, len(item))
		for inner, _ := range item {
...
func GetViewSubGroupOfTitles(mainGroup string, subGroup string) Node {
	list, err := dataExtraction.GetAllInSubGroup(mainGroup, subGroup)
	var newDiv Node = nil
	if err != nil {
		...
		for i, title := range *list {
			holderText := strings.Join(dataValidation.GetDisplayNameArray(&title.Holder), ", ")
